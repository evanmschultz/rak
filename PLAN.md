...
- **Symlinked walk-root normalization (v0.2 hardening)** — surfaced by Drop 4 Unit 4.2 falsification F4 (2026-05-15): if a user passes `rak <symlinked-dir>` where the symlink target sits inside a git repo, `filepath.Abs` does not resolve the symlink while `git rev-parse --show-toplevel` does, so `absRoot` and `toplevel` diverge and `GitLister.List` silently yields zero results. Rak's own checkout doesn't repro it (no symlinked walk roots in dev use). Defer to v0.2 hardening: either add `filepath.EvalSymlinks` normalization to `newGitLister`, or document the limitation in `--help` / README so users resolve the symlink before invoking.
- **Default-TOON path-arg integration coverage** — surfaced by Drop 4 Unit 4.4 falsification F1 (2026-05-15): existing integration tests in `cmd/rak/integration_test.go` cover `--human` and `--json` (both stdin + path-arg) but no test exercises the most-common user invocation `rak <dir>` with no flag (default TOON + GitLister composition). The three components are tested in isolation (Unit 4.2 GitLister, Unit 4.5 TOON renderer, Unit 4.4 runDirectory stdin-default-TOON) but their composition is unverified end-to-end. Fold into Drop 9.1 README polish work, or add as a one-off test in Drop 5 when integration tests are next touched.
- **render.go package doc missing NewTOONRenderer** — surfaced by Drop 4 Unit 4.4 falsification F2 (2026-05-15): `internal/render/render.go` package doc lines 1–7 list only `NewHumanRenderer` and `NewJSONRenderer`. Constructor surface should include `NewTOONRenderer`. One-line doc fix. Fold into Drop 9 README/doc polish.
- **`runDirectory` 10-param signature refactor (v0.2)** — surfaced by Drop 8 Unit 8.1 falsification (2026-05-15): `cmd/rak.runDirectory` grew to 10 parameters across Drops 4/5/7/8. Functional but smelly. Candidate for an options-struct refactor in v0.2: `runDirectory(ctx, src, opts runDirectoryOpts)` where `opts` carries `rootLabel, binary, langs, sortKey, sortAsc, maxFiles, renderer`. YAGNI for v0.1.0; refactor when next feature pressure pushes the param count further.
