...
- **`rootFlags` field reshape (remove `format`, add `human/json/toon bool`)** — **pass**. `cmd/rak/root.go` L23-33 declares the struct: `human bool`, `json bool`, `toon bool` are the first three fields, replacing the previous `format string`. `git diff HEAD~1 -- cmd/rak/root.go` confirms `-format string` removed and `+human/+json/+toon bool` added. Order matches PLAN.md acceptance verbatim.
- **`newRootCmd` flag registration (delete `--format`/`-f`, add three bool flags)** — **pass**. `root.go` L56-73 binds `--human`, `--json`, `--toon` via three `cmd.Flags().BoolVar` calls (no shorthand letter — none specified in PLAN.md). The previous `cmd.Flags().StringVarP(..., "format", "f", "auto", ...)` block is deleted (diff `-StringVarP` block at the same offset). No `-f` short flag remains.
- **`cmd.MarkFlagsMutuallyExclusive("human", "json", "toon")` wired** — **pass**. `root.go` L74: `cmd.MarkFlagsMutuallyExclusive("human", "json", "toon")` — exactly three flag names, in the order required. F24 satisfied. Wired immediately after the three `BoolVar` registrations so all flags are bound before mutex declaration (cobra requires registration before mutex).
- **`resolveRenderer(flags *rootFlags) render.Renderer` (no error return)** — **pass**. `root.go` L121-130 defines the function with signature `func resolveRenderer(flags *rootFlags) render.Renderer` — single return value, no error. Body is a 3-arm `switch { case flags.human: ... case flags.json: ... default: ... }`. The `default` branch returns `render.NewTOONRenderer()`, covering both "no flag set" and "--toon explicit" cases per F24 (cobra mutex guarantees at most one is true). Doc comment L116-120 cites F24 and decision 33 (TOON as default).
- **`runRoot` calls `lister.Detect` + handles `ErrNoGitignoreInRepo` sentinel (F19)** — **pass**. `root.go` L154-162: `len(args) == 1` branch invokes `source, err := lister.Detect(ctx, args[0], listerOpts(flags))`, then `if err != nil { return err }` — the error is propagated to cobra unmodified. **Sentinel chain trace:** `internal/lister/lister.go` L67-68 returns `fmt.Errorf("lister: detect: %w", ErrNoGitignoreInRepo)` (verb `%w` preserves the chain). `runRoot` returns that wrapped error verbatim. Cobra's `Execute()` surfaces it. Consumers calling `errors.Is(err, lister.ErrNoGitignoreInRepo)` traverse one wrap layer — `errors.Is` is recursive, the sentinel matches. `cmd/rak/root_test.go` L176-178 asserts this end-to-end via `isErrNoGitignoreInRepo(err)` which calls `errors.Is(err, lister.ErrNoGitignoreInRepo)`.
- **`runDirectory` new signature `(ctx, w io.Writer, source lister.FileLister, rootLabel, binary, renderer)`** — **pass with note**. `root.go` L183-190: the signature is `func runDirectory(ctx context.Context, w io.Writer, source lister.FileLister, rootLabel string, binary bool, renderer render.Renderer) error`. **Note:** PLAN.md L120 documents the signature without `w io.Writer`. Builder retained the writer (worklog L418) because `renderer.RenderTree(w, ...)` needs one — the PLAN.md elision was a simplification artifact, not a binding contract. Functionally equivalent to spec; all five other params match exactly: `source lister.FileLister` (replaces `fsys fs.FS`), `binary bool` slot added (replaces `flags *rootFlags`), `rootLabel` + `renderer` unchanged.
- **`walkAndCount` new signature `(ctx, source lister.FileLister, binary bool) → ([]Directory, Counts, []error, error)`** — **pass**. `root.go` L221: `func walkAndCount(ctx context.Context, source lister.FileLister, binary bool) ([]render.Directory, counting.Counts, []error, error)`. Exact signature match. The `fsys fs.FS` parameter is dropped (lister owns it); `flags *rootFlags` collapsed into a single `binary bool` slot. Return tuple unchanged.
- **F23 binary check preserved** — **pass**. `root.go` L243-252 keeps the binary policy verbatim: `if !binary { isBin, err := f.IsBinary(); if err != nil { aggErrs = append(...); continue }; if isBin { continue } }`. Error aggregation pattern unchanged from Drop 3. Worklog § "Integration test fixture verification" line 411 confirms `bin.dat` is binary-skipped in the testdata/tree integration path.
- **`root_test.go` 3 replacements + 2 additions = 5 changed tests** — **pass**.
  - **R1: `TestRootCmd_ReadsStdin_RendersHumanDefault` → `TestRootCmd_ReadsStdin_RendersTOONDefault`** — `root_test.go` L43-63. Asserts default (no flags) emits `bytes:`, `lines:`, `words:`, `chars:` (lowercase TOON keys from `toonCounts` struct tags). Old `Bytes/Lines/Words/Chars` capitalized assertions removed.
  - **R2: `TestRootCmd_FormatJSON` → `TestRootCmd_FlagJSON`** — L69-89. Uses `--json` arg (replaces `--format=json`). Asserts `"Bytes"`, `"Lines"`, `"Words"`, `"Chars"` JSON keys (Counts struct field names, F4 pin).
  - **R3: `TestRootCmd_InvalidFormat` → `TestRootCmd_MutuallyExclusiveFlags`** — L95-118. Sets `--human --json` simultaneously, expects non-nil error containing "human" or "json" flag name (case-insensitive). Robust to cobra's actual wording ("if any flags in the group ... are set none of the others can be").
  - **A1: `TestRootCmd_UnknownFlag`** — L123-137. Sets `--bogus`, expects non-nil error. Single sanity-check assertion.
  - **A2: `TestRootCmd_NoGitignoreInRepo_Errors`** — L145-179. Hermetic: `exec.LookPath("git")` skip-guard at L148-150; `t.TempDir()` + `exec.Command("git", "init", tmpDir)` at L153-157 (skips on `git init` failure). Asserts `errors.Is(err, lister.ErrNoGitignoreInRepo)` via `isErrNoGitignoreInRepo` helper at L176-178. Exercises full pipeline (cobra → `runRoot` → `lister.Detect` → sentinel → cobra error return).
- **`runTreeFS` uses `lister.NewWalkLister` (bypasses Detect)** — **pass**. `root_test.go` L196-208: `source := lister.NewWalkLister(fsys, ".", opts)` constructs the lister directly from the injected `fstest.MapFS`. No `lister.Detect` call, no git probe in unit tests. New `runDirectory` signature is used (passes `source`, `rootLabel=""`, `flags.binary`, `renderer`).
- **Compile-time renderer assertions extended for TOON** — **pass**. `root_test.go` L22-26: var-block now includes `_ render.Renderer = render.NewTOONRenderer()` alongside the two existing assertions. Fails the build if `toonRenderer` ever loses a method.
- **`integration_test.go` `--format=X` → `--X` rewrite (4 occurrences)** — **pass**. `git diff HEAD~1 -- cmd/rak/integration_test.go` confirms exactly 4 substitutions: L60 `--format=human` → `--human` (stdin human), L101 `--format=json` → `--json` (stdin json), L165 `--format=human` → `--human` (path-arg human), L202 `--format=json` → `--json` (path-arg json). Total `+4/-4` matches the stat. No `--format=` substring remains in any file (verified absent in current `integration_test.go`).
- **Integration counts unchanged from Drop 3 (20 bytes / 2 lines / 4 words / 20 chars)** — **pass**. `integration_test.go` L146-150 retains `treeExpectedTotalBytes = 20`, `treeExpectedTotalLines = 2`, `treeExpectedTotalWords = 4`, `treeExpectedTotalChars = 20`. `TestRootCmd_Integration_PathArg_JSONFormat` L213-220 asserts the parsed total against these constants. **Reviewer re-ran `mage test` — `cmd/rak ok (cached)`** — the cache is valid because the only `cmd/rak` change is the bool-flag rewire; `testdata/tree` content is unchanged from Drop 3. Integration test now goes through `lister.Detect` → `GitLister` (testdata/tree is git-tracked); effective file set `a.txt` + `sub/nested.txt` matches the Drop 3 effective set (`.gitignore`/`.hidden.txt` hidden-filtered per F21, `bin.dat` binary-skipped per F23).
- **`mage ci` green (re-run by reviewer)** — **pass**. Output: `0 issues.` followed by `ok` for all 6 packages (`cmd/rak`, `internal/counting`, `internal/fileset`, `internal/ignore`, `internal/lister`, `internal/render`), all cached. No lint, no test failure.
- **F26 RelPath invariant consumer-side preserved** — **pass**. `runDirectory` → `labelDirectories` (L321-337) rewrites `Path == "."` to `rootLabel` and otherwise concatenates `rootLabel + "/" + d.Path`. This relies on `dirs[i].Path` being walk-root-relative, forward-slash, no leading `./` — exactly what `dirKey` (L294-303) produces from `f.RelPath`. Both `WalkLister` and `GitLister` honor F26 at the source (per Unit 4.2/4.3 QA proofs). The Unit 4.4 consumer is correct.
...
None. Acceptance bullets 1-10 from the prompt audit are each backed by a concrete file:line citation + diff verification + mage re-run.

### Summary

PASS — all 14 acceptance bullets verified against `cmd/rak/root.go`, `cmd/rak/root_test.go`, `cmd/rak/integration_test.go`, and `internal/lister/lister.go`; `mage ci` re-runs green; F19 sentinel chain traces cleanly from `Detect` through `runRoot` to cobra; F23 binary check preserved verbatim; F24 mutual-exclusivity wired correctly; F26 RelPath invariant unaffected on the consumer side; integration counts unchanged from Drop 3.

### Hylla Feedback

N/A — Unit 4.4 Round 1 review touched only files modified in HEAD (`cmd/rak/root.go`, `cmd/rak/root_test.go`, `cmd/rak/integration_test.go`) plus the Unit 4.1 lister file (`internal/lister/lister.go` for sentinel-chain trace) which is also post-last-ingest. All evidence gathered via `git diff HEAD~1`, `Read` of source files, and `mage ci` / `mage test` re-runs. Zero Hylla queries attempted; zero misses to report.
