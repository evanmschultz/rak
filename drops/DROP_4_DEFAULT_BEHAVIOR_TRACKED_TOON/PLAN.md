...
- **Acceptance:**
  ...
  - `runRoot`: construct renderer via `resolveRenderer(flags)`. For the `len(args)==1` path, call `lister.Detect(ctx, args[0], listerOpts(flags))` where `listerOpts` translates `rootFlags` fields (`depth`, `hidden`, `noGitignore`, `includes`, `excludes`) to `fileset.WalkOptions`. Pass the resulting `FileLister` into `runDirectory`. Surface `lister.ErrNoGitignoreInRepo` as a normal `RunE` error return — cobra prints the wrapped message. The error message format: `"rak: --no-gitignore has no effect when run inside a git repository. rak counts git-tracked files in this mode. To count untracked files, run rak outside the repository, or wait for the v0.2 --include-untracked flag."` (Decision A — the sentinel's `Error()` carries this message; the `fmt.Errorf("lister: detect: %w", ErrNoGitignoreInRepo)` chain wraps it cleanly).
  - `runDirectory(ctx context.Context, source lister.FileLister, rootLabel string, renderer render.Renderer) error` — accepts `FileLister` instead of `fs.FS`; passes the lister to `walkAndCount`. `rootLabel` and `renderer` parameters unchanged. (P7)
  ...
  - `root_test.go`:
    ...
    - Add `TestRootCmd_NoGitignoreInRepo_Errors` — constructs a `WalkLister` with a TempDir + `git init` as root, invokes `runRoot` with `--no-gitignore`, verifies the error is or wraps `lister.ErrNoGitignoreInRepo`. Actually: since 4.4's unit scope is `cmd/rak`, this test can set up a real temp git repo and call `runRoot` or the cobra cmd with `--no-gitignore` + a path inside it. Alternatively, exercise this through `lister.Detect` (already tested in 4.1's `TestDetect_NoGitignoreInRepo_ReturnsSentinel`); 4.4's test can be a narrower smoke test via the cobra command pipeline. Builder decides cleanest form. Skips if git binary absent.
...
- **F19 — `--no-gitignore` + in-git-repo is a hard error.** When `Detect` runs `git rev-parse --is-inside-work-tree` and gets exit 0 (in repo), it checks `opts.DisableGitignore` immediately. If true, it returns `(nil, ErrNoGitignoreInRepo)` before calling `newGitLister`. This sentinel is declared as `var ErrNoGitignoreInRepo = errors.New(...)` in `lister.go`. Callers use `errors.Is(err, lister.ErrNoGitignoreInRepo)`. The error message text is: `"rak: --no-gitignore has no effect when run inside a git repository. rak counts git-tracked files in this mode. To count untracked files, run rak outside the repository, or wait for the v0.2 --include-untracked flag."` (Decision A.)
