...
**Cascade tiering.** Each drop carries a `Tier:` field in its `PLAN.md` header. Tier **A** = full cascade (planner subagent + parallel plan-QA + parallel build-QA per unit). Tier **B** = lite (orch plans inline, no plan-QA; falsification-only build-QA per unit). Tier **C** = orch-direct (no subagents; dev reviews diff). Full mechanics in `main/drops/WORKFLOW.md` § "Cascade Tiering (A / B / C)".
...
- `internal/fileset/` — `File` type (`Open()`, `Peek(n)`) + `Walker` with `iter.Seq2[*File, error]` + binary detection. Depends on `internal/ignore`.
- `internal/ignore/` — `Matcher` interface unifying gitignore parsing + `--include`/`--exclude` globs. Zero internal deps.
- `internal/lister/` — `FileLister` interface with `GitLister` (shells `git ls-files`) + `WalkLister` (wraps `internal/fileset.Walker`) impls + `Detect(root, fsys)` factory. Lands in Drop 4. Depends on `internal/fileset`.
...
Leaves: `counting`, `ignore`, `summary`. Mid: `fileset` → `ignore`; `lister` → `fileset`; `lang` → `fileset`; `render` → `summary` (post-Drop-7). Root: `cmd/rak` → all of the above. No cycles, strictly layered. (`internal/tokens` deferred to v0.2.)
...
| `cmd/rak/root.go` | cobra root cmd, flags, `RunE` dispatch | ~300 |
| `cmd/rak/root_test.go` | arg/flag parsing + path-arg behavior tests | ~450 |
| `cmd/rak/integration_test.go` | end-to-end fixture tree tests | ~280 |
| `internal/counting/counting.go` | `Counts` struct + `Count(io.Reader)` | ~80 |
| `internal/counting/counting_test.go` | ASCII, UTF-8, CRLF, empty | ~80 |
| `internal/fileset/file.go` | `File` type, `Open()`, `Peek(n)`, `IsHidden` helper | ~130 |
| `internal/fileset/file_test.go` | `Open`, `Peek`, `IsHidden` coverage | ~180 |
| `internal/fileset/walker.go` | `Walker` + `Walk() iter.Seq2[*File, error]` | ~350 |
| `internal/fileset/walker_test.go` | walks against `fstest.MapFS` | ~520 |
| `internal/fileset/binary.go` | `ErrBinaryFile` + `(*File).IsBinary` via NUL-in-Peek(512) | ~80 |
| `internal/fileset/binary_test.go` | NUL-byte detection table | ~100 |
| `internal/ignore/ignore.go` | `Matcher` interface + constructor | ~120 |
| `internal/ignore/gitignore.go` | gitignore parsing (library wrap) | ~100 |
| `internal/ignore/glob.go` | `--include`/`--exclude` globs | ~60 |
| `internal/ignore/ignore_test.go` | match tables | ~350 |
| `internal/lister/lister.go` | `FileLister` interface + `Detect(root, fsys)` factory | ~80 |
| `internal/lister/git.go` | `GitLister` — shells `git ls-files --full-name -z` | ~150 |
| `internal/lister/walk.go` | `WalkLister` — adapter over `internal/fileset.Walker` | ~50 |
| `internal/lister/lister_test.go` | factory routing + impl tests (real-git smoke + MapFS fallback) | ~250 |
...
| `internal/render/render.go` | `Renderer` interface | ~60 |
| `internal/render/toon.go` | TOON renderer (default; via toon-format/toon-go) | ~150 |
| `internal/render/human.go` | laslig TTY renderer | ~130 |
| `internal/render/json.go` | JSON renderer | ~80 |
| `internal/render/render_test.go` | snapshot tests per format | ~400 |
...
| `magefile.go` | mage targets | ~130 |

Non-test Go: ~2,250 LOC. Test Go: ~3,100 LOC. Total v0.1.0: ~5,350 LOC. (Original 3,100 estimate undershot — Drop 3's walker testing alone was 520 LOC, and adding lister + TOON in Drop 4 grows render-test surface materially.)
...
- `github.com/evanmschultz/laslig` — structured output for the `--human` opt-in renderer.
- `github.com/toon-format/toon-go` — TOON encoder for the default LLM-first renderer (added in Drop 4).
- `github.com/sabhiram/go-gitignore` — `.gitignore` pattern matching for the not-in-git fallback walker (added in Drop 3.0).
- `github.com/bmatcuk/doublestar/v4` — `--include`/`--exclude` glob matching (added in Drop 3.0).
...
- `chore(deps): add toon-format/toon-go`
